...
Here, we instead calculate roughness independently of slope via the dispersion of orthogonal residuals from a plane fitted through a given DEM point and its neighbours. To do so, we use SVD, which we apply, likewise to the previous DEM-based methodologies, using a sliding-window approach.
